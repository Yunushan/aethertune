...
void main() {
  test('rejects the deployment operations-token placeholder', () {
    expect(
      () => StaticOperationsAuthenticator(operationsTokenPlaceholder),
  ...
  test(
    'default managed tokens carry 256 bits of secure random material',
    () async {
  ...
  test('managed token expiry is opt-in and enforced at its boundary', () async {
  ...
  test('recovery code is single-use and replaces every device token', () async {
    ...
    expect(
      registry.account('primary')!.tokens.single.deviceName,
      'Replacement phone',
    );
    expect(
  ...
  test(
    'recovery code remains digest-only and redeemable after restart',
    () async {
  ...
  test('validates the managed token lifetime environment setting', () {
    ...
    expect(
      managedTokenLifetimeFromEnvironment(const <String, String>{
        'AETHERTUNE_MANAGED_TOKEN_TTL_DAYS': '30',
      }),
  ...
  test(
    'records managed device activity at a bounded durable cadence',
    () async {
  ...
  test(
    'managed profile updates persist without changing token identity',
    () async {
      final root = await Directory.systemTemp.createTemp(
      ...
      await registry.issueToken(accountId: 'primary', deviceName: 'Desktop');
  ...
  test(
    'failed managed profile persistence leaves active state unchanged',
    () async {
      final root = await Directory.systemTemp.createTemp(
      ...
      await Directory(
        '${root.path}${Platform.pathSeparator}registry-2.json',
      ).create();
  ...
  test(
    'managed registry persists one-time device tokens and lifecycle changes',
    () async {
      final root = await Directory.systemTemp.createTemp(
      ...
      final storedAfterIssue = await (await _onlyRegistryFile(
        root,
      )).readAsString();
  ...
  test(
    'managed admin APIs issue, rotate, revoke, and share account snapshots',
    () async {
      final generated = Queue<String>.from(<String>[
        'at_api_phone_secret',
      ...
      expect(profileBody['account'], <String, Object?>{
        'id': 'primary',
        'displayName': 'Primary listener',
        'avatarTone': null,
      ...
      expect((profileUpdateBody['device'] as Map)['deviceName'], 'Workstation');
      ...
      expect(
        publicProfilePage.headers['content-type'],
      ...
      final nameOnlyAccount =
          (await _json(nameOnlyPublicProfile))['account']
      ...
      expect((phoneProfileBody['device'] as Map)['deviceName'], 'Phone');
      ...
      expect(
        escapedProfilePageBody,
        contains('&lt;img src=x onerror=alert(1)&gt;'),
      );
      expect(
        escapedProfilePageBody,
        isNot(contains('<img src=x onerror=alert(1)>')),
      );
      ...
      final accountList = await handler(
        _request('GET', '/api/v1/admin/sync-accounts', token: 'ops-secret'),
      );
      final accountListText = await accountList.readAsString();
      final accountListBody =
          jsonDecode(accountListText) as Map<String, dynamic>;
      final listedAccount =
          (accountListBody['accounts'] as List<dynamic>).single as Map;
      expect(listedAccount['displayName'], 'Shared listeners');
      expect(
        (listedAccount['tokens'] as List<dynamic>).map(
          (token) => (token as Map)['deviceName'],
        ),
  ...
  test('managed administration fails closed without operations auth', () async {
    ...
    expect((await _json(response))['error'], 'operations_auth_not_configured');
    expect(accounts.isConfigured, isFalse);
  });

  test(
    'static sync credentials cannot mutate managed profile metadata',
    () async {
      final authenticator = StaticSyncAuthenticator.fromJson(
        '{"static-account":"static-token"}',
      );
      final response =
          await createServerHandler(syncAuthenticator: authenticator)(
            _request(
              'PATCH',
              '/api/v1/auth/profile',
              token: 'static-token',
  ...
  test(
    'static identity wins without managed rights for an overlapping token',
    () async {
      final accounts = ManagedSyncAccountRegistry.memory(
      ...
      final authenticator = CompositeSyncAuthenticator(<SyncAuthenticator>[
        staticAccounts,
        accounts,
      ]);
      final handler = createServerHandler(
      ...
      final profile = await handler(
        _request('GET', '/api/v1/auth/profile', token: 'overlapping-token'),
      );
      final profileBody = await _json(profile);
      ...
      expect(
        accounts.account('managed-account')?.displayName,
        'Managed account',
      );
    },
  );
