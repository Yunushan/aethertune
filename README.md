...
| Provider architecture | `MusicSourceProvider` interface with capability flags, privacy/network disclosure, demo provider, Podcast RSS feeds, and Radio Browser station search/filtering |
...
| Multi-source | Local provider support plus Podcast RSS feed subscriptions/play/save/OPML/refresh status/progress resume and Radio Browser station search/filter/play/save/click accounting; self-hosted, open catalog, and official API providers remain roadmap |
