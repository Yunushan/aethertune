...
| Provider architecture | `MusicSourceProvider` interface with capability flags, privacy/network disclosure, offline cache/download policy gates, persisted offline request queue, user-triggered private cache storage and configurable-limit eviction for direct media URLs, unified provider search, demo provider, Podcast RSS feeds, Radio Browser mirror discovery/search/filtering/stream validation, and Internet Archive audio search/filtering |
...
| Offline | Local-first data model, offline library filter, persisted offline mode that pauses network-backed source actions and player-wide saved stream playback, per-provider cache/download policy gate, persisted cache/download queue manager, private direct-URL cache storage with usage/trim/clear/configurable-limit controls; resumable/background downloads, automatic pressure eviction, and per-provider quotas remain roadmap |
