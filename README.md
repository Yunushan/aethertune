...
| Provider architecture | `MusicSourceProvider` interface with capability flags, privacy/network disclosure, offline cache/download policy gates, persisted offline request queue, user-triggered checksum-verified private cache storage plus automatic app/provider quota eviction for direct media URLs, unified provider search, demo provider, Podcast RSS feeds, Radio Browser mirror discovery/search/filtering/stream validation, and Internet Archive audio search/filtering |
...
| Offline | Local-first data model, offline library filter, persisted offline mode that pauses network-backed source actions and player-wide saved stream playback, per-provider cache/download policy gate, persisted cache/download queue manager, checksum-verified private direct-URL cache storage with usage/trim/clear/app-provider quota controls and automatic post-cache pressure eviction; resumable/background downloads remain roadmap |
