...
| Music discovery | Local Home feed sections, user-refreshed self-hosted discovery lists (Jellyfin recently added albums; Navidrome/Subsonic newest, frequently played, recently played, and random albums; provider playlists), followed-local-artist updates, range-filtered local charts, local mood/activity mixes, explainable personalized local recommendations with independent favorite/history controls, similar local tracks, related local artist/album pages, seed-based track radio, and collection-aware artist/album radio queues implemented; remote creator/channel subscriptions, provider editorial/personalized recommendation feeds, provider charts, provider moods, provider-backed radio, and provider-backed artist/album recommendations remain roadmap |
...
| Android integrations | Queue-aware media notification, artwork/metadata, play/pause/seek/previous/next/stop, repeat/shuffle, media-button receiver, foreground media service, and background playback configuration implemented. The generated home-screen widget provides previous, play/pause, and next through that same media-button receiver, mirrors the current title, artist, and play/pause state, and the launcher exposes static Previous, Play/Pause, and Next shortcuts through the same route; physical-device lifecycle tests, widget artwork, user-pinned shortcuts, and Android Auto browsing remain roadmap |
