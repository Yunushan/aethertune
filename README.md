...
| Provider architecture | `MusicSourceProvider` interface with capability flags, privacy/network disclosure, demo provider, Podcast RSS feeds, Radio Browser station search/filtering, and Internet Archive audio search |
...
| Multi-source | Local provider support plus Podcast RSS feed subscriptions/play/save/OPML/refresh status/progress resume, Radio Browser station search/filter/play/save/click accounting, and Internet Archive audio search/play/save; self-hosted and official API providers remain roadmap |
