...
/// Converts an application media id into an MPRIS-safe, stable object path.
DBusObjectPath mprisTrackPathForId(String mediaId) {
...
class OrgMprisMediaPlayer2 extends DBusObject {
  ...
  void setTracks(Iterable<MprisTrack> tracks) {
    _tracks
      ..clear()
      ..addAll(tracks);
    unawaited(
      emitSignal(
        ...
        'TrackListReplaced',
        <DBusValue>[
          DBusArray.objectPath(_tracks.map((track) => track.path)),
          _metadata.trackId ?? _noTrackPath,
        ],
      ),
    );
  }
  ...
  Future<DBusMethodResponse> doGetTracksMetadata(
    Iterable<DBusObjectPath> paths,
  ) async {
    final metadataByPath = <String, Metadata>{
      for (final track in _tracks) track.path.value: track.metadata,
    };
    final metadata = <DBusValue>[];
    for (final path in paths) {
      final value = metadataByPath[path.value];
      if (value == null) {
        return DBusMethodErrorResponse.invalidArgs();
      }
      metadata.add(value.toValue());
    }
    return DBusMethodSuccessResponse([
      DBusArray(DBusSignature('a{sv}'), metadata),
    ]);
  }

  Future<DBusMethodResponse> doGoTo(DBusObjectPath path) async {
    for (final track in _tracks) {
      if (track.path == path) {
        _trackStreamController.add(track.mediaId);
        return DBusMethodSuccessResponse([]);
      }
    }
    return DBusMethodErrorResponse.invalidArgs();
  ...
  List<DBusIntrospectInterface> introspect() {
    return [
      ...
      DBusIntrospectInterface('org.mpris.MediaPlayer2.TrackList', methods: [
        DBusIntrospectMethod('GetTracksMetadata', args: [
          DBusIntrospectArgument(DBusSignature('ao'), DBusArgumentDirection.in_,
              name: 'TrackIds'),
          DBusIntrospectArgument(
              DBusSignature('aa{sv}'), DBusArgumentDirection.out,
              name: 'Metadata'),
        ]),
        DBusIntrospectMethod('GoTo', args: [
          DBusIntrospectArgument(DBusSignature('o'), DBusArgumentDirection.in_,
              name: 'TrackId'),
        ]),
      ], signals: [
        DBusIntrospectSignal('TrackListReplaced', args: [
          DBusIntrospectArgument(DBusSignature('ao'), DBusArgumentDirection.out,
              name: 'Tracks'),
          DBusIntrospectArgument(DBusSignature('o'), DBusArgumentDirection.out,
              name: 'CurrentTrack'),
        ]),
      ], properties: [
        DBusIntrospectProperty('Tracks', DBusSignature('ao'),
            access: DBusPropertyAccess.read),
  ...
  Future<DBusMethodResponse> handleMethodCall(DBusMethodCall methodCall) async {
    ...
    } else if (methodCall.interface == 'org.mpris.MediaPlayer2.TrackList') {
      if (methodCall.name == 'GetTracksMetadata') {
        if (methodCall.signature != DBusSignature('ao')) {
          return DBusMethodErrorResponse.invalidArgs();
        }
        return doGetTracksMetadata(
          methodCall.values[0].asArray().map((value) => value.asObjectPath()),
        );
  ...
  Future<DBusMethodResponse> getProperty(String interface, String name) async {
    ...
    if (interface == 'org.mpris.MediaPlayer2') {
      ...
      if (name == 'Tracks') {
        return DBusMethodSuccessResponse([DBusVariant(getTracks())]);
      }
      if (name == 'CanEditTracks') {
        return DBusMethodSuccessResponse(
