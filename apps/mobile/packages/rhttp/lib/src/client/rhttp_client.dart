...
class RhttpClient {
  ...
  /// Signals cancellation before the caller drains outstanding response streams.
  /// Unlike a request token, this also completes when no request was started.
  Future<void> cancelRunningRequests() async {
    if (!ref.isDisposed) {
      await rust.cancelRunningRequests(client: ref);
    }
  }
