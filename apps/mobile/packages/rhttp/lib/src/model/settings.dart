...
const _keepMaxStreamResponseBytes = -1;
...
class ClientSettings {
  ...
  /// Maximum decoded stream-response bytes queued by the native client.
  /// Null preserves the upstream unlimited policy. Zero permits an empty body.
  ...
  ClientSettings copyWith({
    ...
    int? maxStreamResponseBytes = _keepMaxStreamResponseBytes,
  }) {
    return ClientSettings(
      ...
      maxStreamResponseBytes:
          maxStreamResponseBytes == _keepMaxStreamResponseBytes
          ? this.maxStreamResponseBytes
          : maxStreamResponseBytes,
    );
  }
...
extension ClientSettingsExt on ClientSettings {
  rust_client.ClientSettings toRustType() {
    final limit = maxStreamResponseBytes;
    if (limit != null && (limit < 0 || limit > 0xffffffff)) {
      throw ArgumentError.value(limit, 'maxStreamResponseBytes');
    }
