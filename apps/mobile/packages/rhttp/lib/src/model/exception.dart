...
/// A response exceeded the configured native decoded-stream limit.
class RhttpResponseTooLargeException extends RhttpException {
  const RhttpResponseTooLargeException(super.request);

  @override
  String toString() => '[$runtimeType] Response body exceeds its byte limit.';
}
...
RhttpException parseError(HttpRequest request, rust.RhttpError error) {
  return switch (error) {
    ...
    rust.RhttpError_RhttpUnknownError(field0: 'RHTTP_STREAM_RESPONSE_LIMIT') =>
      RhttpResponseTooLargeException(request),
