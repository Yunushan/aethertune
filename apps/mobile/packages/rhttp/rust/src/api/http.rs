...
impl HttpMethod {
    fn to_method(&self) -> Result<Method, RhttpError> {
        Method::from_bytes(self.method.as_bytes())
            .map_err(|_| RhttpError::RhttpUnknownError("Invalid HTTP method".to_string()))
    }
}
...
async fn make_http_request_receive_stream_inner(
    ...
    on_response: impl Fn(HttpResponse) -> DartFnFuture<()>,
    on_error: &impl Fn(RhttpError) -> DartFnFuture<()>,
    cancel_tokens: RequestCancelTokens,
) {
    let max_bytes = match client.as_ref() {
        Some(client) => client.try_read().unwrap().max_stream_response_bytes,
        None => settings
            .as_ref()
            .and_then(|settings| settings.max_stream_response_bytes),
    };
    // Cancellation may drop network futures, but never an in-flight Dart callback.
    let response = tokio::select! {
        biased;
        _ = cancel_tokens.request_cancel_token.cancelled() => Err(RhttpError::RhttpCancelError),
        _ = cancel_tokens.client_cancel_token.cancelled() => Err(RhttpError::RhttpCancelError),
        response = make_http_request_helper(
            client, settings, method, url, query, headers, body, body_stream, None,
        ) => response,
    };
    ...
    if let (Some(limit), Some(length)) = (max_bytes, response.content_length()) {
        if length > u64::from(limit) {
            on_error(RhttpError::RhttpUnknownError(
                error::STREAM_RESPONSE_LIMIT.into(),
            ))
            .await;
            return;
        }
    ...
    let mut limit = StreamResponseLimit::new(max_bytes);
    ...
    loop {
        ...
        // Bound decoded bytes before copying them into the Dart event queue.
        if !limit.accept(chunk.len()) {
            let _ = stream_sink.add_error(anyhow::anyhow!(error::STREAM_RESPONSE_LIMIT));
            return;
        }
...
struct StreamResponseLimit {
    remaining: Option<usize>,
}

impl StreamResponseLimit {
    fn new(limit: Option<u32>) -> Self {
        Self {
            remaining: limit.map(|value| value as usize),
        }
    }

    fn accept(&mut self, length: usize) -> bool {
        match self.remaining {
            None => true,
            Some(remaining) if length <= remaining => {
...
mod response_limit_tests {
    use super::StreamResponseLimit;

    #[test]
    fn accepts_exact_boundary_and_rejects_the_next_byte() {
        let mut limit = StreamResponseLimit::new(Some(9));
        assert!(limit.accept(4));
        assert!(limit.accept(5));
        assert!(limit.accept(0));
        assert!(!limit.accept(1));
    }

    #[test]
    fn rejects_oversized_chunks_without_integer_overflow() {
        let mut limit = StreamResponseLimit::new(Some(9));
        assert!(!limit.accept(usize::MAX));
        assert!(limit.accept(9));
        assert!(!StreamResponseLimit::new(Some(0)).accept(1));
    }

    #[test]
    fn leaves_unconfigured_streams_unlimited() {
        let mut limit = StreamResponseLimit::new(None);
        assert!(limit.accept(usize::MAX));
        assert!(limit.accept(usize::MAX));
    }
}
        ...
        request = match body {
            ...
            Some(HttpBody::BytesStream) => {
                let stream = body_stream
                    .ok_or_else(|| {
                        RhttpError::RhttpUnknownError("Missing request body stream".to_string())
                    })?
                    .receiver
    ...
    let response = client.client.execute(request).await.map_err(|e| {
        if e.is_redirect() {
            ...
            if is_cert_error {
                ...
                RhttpError::RhttpConnectionError(match inner {
