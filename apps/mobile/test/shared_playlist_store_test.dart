...
void main() {
  ...
  test('hosts and explicitly publishes a private shared playlist', () async {
    ...
    await library.replacePlaylistTracks(playlist.id, const <String>[
      'one',
      'two',
      'one',
    ]);
  ...
  test(
    'joins an invite as a local playlist and preserves repeated tracks',
    () async {
      final library = await _libraryWithTracks();
      final gateway = _MemorySharedPlaylistGateway()
        ..remote = _remote(
  ...
  test(
    'owners can revoke a collaborator and retain the new revision',
    () async {
      final library = await _libraryWithTracks();
      final playlist = await library.createPlaylist(
        'Road mix',
  ...
  test(
    'restores an earlier revision as a new shared playlist revision',
    () async {
      final library = await _libraryWithTracks();
      final playlist = await library.createPlaylist(
        'Current mix',
      ...
      expect(library.playlistById(playlist.id)?.trackIds, <String>[
        'one',
        'one',
      ]);
    },
  ...
  test(
    'merges current server order with local-only track occurrences',
    () async {
      final library = await _libraryWithTracks();
      final playlist = await library.createPlaylist(
        'Local mix',
  ...
  test(
    'resolves only unambiguous portable shared tracks in another library',
    () async {
  ...
  test(
    'merges portable shared track references without dropping duplicates',
    () {
      ...
      expect(merged.map((reference) => reference.title), <String>[
        'One',
        'Two',
        'One',
        'One',
  ...
  test(
    'adds only a single exact provider match for an unavailable shared track',
    () async {
      ...
      expect(provider.queries, <String>[
        'Provider song Provider artist Provider album',
      ]);
  ...
  test(
    'keeps provider results unavailable when an exact match is ambiguous',
    () async {
      final library = LibraryStore();
      await library.load();
      final gateway = _MemorySharedPlaylistGateway()
  ...
  test(
    'does not make a resolved duration-less shared track ambiguous',
    () async {
      final library = LibraryStore();
      await library.load();
      ...
      expect(result.resolvedTrackCount, 0);
      expect(result.unavailableTrackCount, 1);
      expect(library.tracks.map((track) => track.id), <String>[
        'existing-song',
      ]);
      expect(library.playlistById(binding.localPlaylistId)?.trackIds, <String>[
        'existing-song',
  ...
  test(
    'queues only policy-approved provider tracks for shared downloads',
    () async {
      final library = LibraryStore();
      await library.load();
      await library.addTracks(<Track>[
        ...
        Track(id: 'local', title: 'Local', localPath: '/library/local.mp3'),
      ]);
      final playlist = await library.createPlaylist(
        'Shared downloads',
        trackIds: const <String>[
      ...
      expect(
        library.offlineCacheQueue.single.action,
