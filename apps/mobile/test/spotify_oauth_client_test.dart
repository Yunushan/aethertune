...
void main() {
  test('builds the RFC PKCE S256 challenge', () {
    expect(
      spotifyPkceChallenge('dBjftJeZ4CVP-mB92K27uhbUJU1p1r_wW1gFWFOEjXk'),
      'E9Melhoa2OwvFrEMTJguCHaoeK1t8URWbuGJSstw-cM',
    );
  });

  test(
    'creates an authorization URL with state, PKCE, and loopback callback',
    () {
  ...
  test('deduplicates and requests explicit Spotify OAuth scopes', () {
    final request = SpotifyAuthorizationRequest.create(
      clientId: 'client-id',
      redirectUri: Uri.parse('http://127.0.0.1:45678/spotify-callback'),
      scopes: const <String>['user-library-read', ' user-library-read ', ''],
    );
  ...
  test(
    'requests read-only Spotify library, playlist, history, and follow scopes',
    () {
      expect(
      ...
      expect(
        ...
        'user-library-read playlist-read-private user-read-recently-played user-top-read user-follow-read',
      );
    },
  );
  ...
  test(
    'retains the old refresh token when Spotify omits a replacement',
    () async {
      final client = SpotifyOAuthClient(
        clock: () => DateTime.utc(2026, 7, 17, 12),
