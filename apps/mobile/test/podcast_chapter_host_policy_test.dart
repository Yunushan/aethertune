...
void main() {
  ...
  test(
    'records bounded approval history for each podcast subscription',
    () async {
      ...
      expect(
        ...
        isA<PodcastChapterHostApproval>()
            .having((entry) => entry.host, 'host', 'chapters.example.test')
            .having((entry) => entry.approvedAt, 'approvedAt', now),
      );
      expect(
  ...
  test(
    'migrates host-only preferences and bounds subscription history',
    () async {
      SharedPreferences.setMockInitialValues(<String, Object>{
        'aethertune.podcast_chapter_hosts.v1': <String>['legacy.example.test'],
      });
      final policy = PodcastChapterHostPolicy();
      await policy.load();
