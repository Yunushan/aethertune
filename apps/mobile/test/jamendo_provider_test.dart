...
void main() {
  test(
    'searches the documented Jamendo endpoint with bounded paging',
    () async {
      Uri? requested;
  ...
  test(
    'loads a documented featured-genre chart with a popularity boost',
    () async {
      Uri? requested;
      final provider = JamendoProvider(
        clientId: 'client-id',
  ...
  test('validates Jamendo payloads and rejects unsafe media URLs', () {
    ...
    expect(
      ...
      Uri.parse('https://art.example.test/1.jpg'),
    );
  ...
  test(
    'browses and explicitly searches bounded Jamendo catalog collections',
    () async {
      final requests = <Uri>[];
      final provider = JamendoProvider(
        clientId: 'client-id',
  ...
  test(
    'loads nested Jamendo artist albums and album tracks with safe artwork',
    () async {
      final requests = <Uri>[];
      ...
      expect(
        album.tracks.single.streamUrl,
        'https://stream.example.test/72.mp3',
      );
      ...
      expect(
        playlistRequest.queryParameters['track_type'],
        'single albumtrack',
      );
      expect(
        await provider.loadArtwork('http://art.example.test/unsafe.jpg'),
        isNull,
      );
    },
  );
...
String _response(List<String> ids) {
  final rows = ids
      .map(
        (id) =>
            '{"id":"$id","name":"Track $id","artist_name":"Artist","album_name":"Album","duration":180,"audio":"https://stream.example.test/$id.mp3"}',
      )
      .join(',');
