...
void main() {
  test(
    'stores an authorized Spotify session only in the credential vault',
    () async {
  ...
  test(
    'refreshes an expired token and atomically persists the replacement',
    () async {
      final vault = _MemoryVault()
      ...
      final store = SpotifySettingsStore(
        ...
        oauthClient: SpotifyOAuthClient(
          ...
          request:
              (uri, {required method, required headers, String? body}) async {
                refreshCalls += 1;
                expect(uri, SpotifyOAuthClient.tokenUri);
                expect(method, 'POST');
                expect(
                  Uri.splitQueryString(body!)['grant_type'],
                  'refresh_token',
                );
