...
void main() {
  ...
  test('normalizes a declared custom catalog and rejects unconsented HTTP', () {
    ...
    expect(definition.declaredNetworkDomains, <String>[
  ...
  test(
    'searches a declared catalog and only resolves declared media hosts',
    () async {
      final definition = CustomCatalogDefinition.create(
        id: 'catalog-003',
        name: 'Community mixes',
      ...
      expect(
        provider.disclosure.networkDomains,
        definition.declaredNetworkDomains,
      );
      expect(provider.disclosure.dataSent, const <String>[
  ...
  test('pages and suggests bounded matching catalog tracks', () async {
    ...
    expect(requests, 3);
    await expectLater(
      provider.searchPage('drive', limit: 0),
      throwsArgumentError,
    );
  ...
  test(
    'persists bounded custom catalog definitions without provider secrets',
    () async {
      ...
      expect(
        restoredDefinition.catalogUri.toString(),
        definition.catalogUri.toString(),
      );
  ...
  test(
    'moves only credential-free HTTPS catalog definitions between devices',
    () async {
