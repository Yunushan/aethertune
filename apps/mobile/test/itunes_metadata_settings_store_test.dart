...
void main() {
  ...
  test(
    'persists a validated storefront and recreates the search provider',
    () async {
      ...
      expect(
        (restored.musicProviders.single as ItunesMetadataProvider).country,
        'tr',
      );
    },
  );
  ...
  test(
    'rejects malformed storefronts without replacing a saved value',
    () async {
      final store = ItunesMetadataSettingsStore();
      await store.load();
  ...
  test(
    'falls back safely when persisted storefront data is malformed',
    () async {
      SharedPreferences.setMockInitialValues(<String, Object>{
