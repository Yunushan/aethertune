...
void main() {
  test(
    'loads a bounded page of account playlists with a bearer token',
    () async {
      Uri? requestedUri;
      String? token;
  ...
  test(
    'loads a bounded page of account subscriptions with a bearer token',
    () async {
      Uri? requestedUri;
      final provider = YouTubeAccountProvider(
        accessTokenReader: () async => 'access-token',
  ...
  test(
    'loads the newest metadata for an account subscription channel',
    () async {
      Uri? requestedUri;
      final provider = YouTubeAccountProvider(
        accessTokenReader: () async => 'access-token',
