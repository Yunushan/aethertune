...
void main() {
  test(
    'sends a bounded LibreTranslate request and returns translated text',
    () async {
      Uri? requestedUri;
      ...
      expect(
        requestedUri,
        Uri.parse('https://translate.example.test/api/translate'),
      );
  ...
  test(
    'rejects invalid endpoints, languages, responses, and oversized lyrics',
    () async {
      expect(
      ...
      expect(
        () => normalizeTranslationLanguage('english'),
        throwsFormatException,
      );

      final translator = LibreTranslateLyricsTranslator(
        baseUri: Uri.parse('https://translate.example.test'),
        responseLoader: (_, _, _) async =>
            const LyricsTranslationResponse(statusCode: 200, body: '{}'),
      );
      await expectLater(
