...
void main() {
  test(
    'requests and parses bounded public podcast directory results',
    () async {
      Uri? capturedUri;
      ...
      expect(
        results.first.feedUri.toString(),
        'https://feeds.example.test/aether',
      );
      expect(results.last.title, 'Signal Room');

      expect(await directory.search('   '), isEmpty);
      await expectLater(
        directory.search('science', limit: 0),
        throwsArgumentError,
      );
    },
  );
