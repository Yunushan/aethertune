...
void main() {
  ...
  test(
    'portable snapshot excludes device paths cache jobs and secret URLs',
    () async {
      ...
      final playlist =
          (snapshot['playlists'] as List<dynamic>).single
  ...
  test(
    'restore reattaches local files and preserves device cache settings',
    () async {
      ...
      await remoteStore.setLyrics(remoteTrack.id, 'Synced lyrics');
      ...
      expect(
        localStore.lyricsForTrack('remote-track-id')?.plainText,
        'Synced lyrics',
      );
      expect(
