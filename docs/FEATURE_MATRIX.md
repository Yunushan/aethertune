...
| Podcast RSS subscriptions | Scaffolded | Sources tab adds/removes persisted RSS feed subscriptions, imports/exports OPML, loads playable episodes, tracks refresh status/staleness, plays/saves episodes, queues cache/download requests after provider policy approval, materializes direct enclosure URLs into checksum-verified private cache storage with usage/app-provider quota eviction controls, resumes saved episode progress, includes backups, declares cache/download policy for legal enclosures, and provider/cache/store behavior has tests. | Background/resumable downloads. |
...
| Stream cache | Scaffolded | InnerTune, RiMusic, YouTube Music | Provider-approved cache requests can be queued, managed, downloaded into private app storage for direct HTTP(S) media URLs, checksum-verified after write, stored with byte-count/checksum metadata, exported to a user-chosen folder after verification, measured, trimmed/cleared to configurable app/provider size limits, automatically evicted after cache writes when over limit, and replayed as local files. Background/resumable fetches remain. |
| Download queue | Scaffolded | YouTube Music, NewPipe | Sources tab queues legal downloads only after `OfflineMediaPolicy` allows them, persists the queue with cache metadata, exposes remove/clear/cache/export management in Options, materializes direct URLs into checksum-verified private storage, can export verified cached files to a user-chosen folder, and can evict private cached files. Background/resumable jobs and system Downloads integration remain. |
...
| Offline mode toggle | Done | YouTube Music | Persisted Options toggle pauses network-backed Sources searches, feed refreshes, Sources stream playback actions, and player-wide saved URL stream playback; backup/restore preserves it, Library can filter to local offline-ready files, and Options exposes the offline queue/cache manager with usage/app-provider quota eviction controls and automatic post-cache pressure eviction. Needed next: background/resumable downloads. |
| Partial/resumable downloads | Roadmap | NewPipe-style clients | Downloader with resume/background jobs and provider-supplied checksum support where available; private direct-URL writes already get post-write checksum verification. |
...
| Podcast RSS | Scaffolded | YouTube Music podcasts, NewPipe | RSS parser/provider, persisted feed subscriptions, OPML import/export, refresh status/stale policy, episode listing, playback, saved episode progress/resume, library save, backup/restore, cache/download policy declarations, queued offline requests, checksum-verified private direct-enclosure caching, and app/provider quota private cache eviction are implemented; background/resumable downloads remain. |
...
7. Expand the current checksum-verified direct-URL offline cache with user-chosen folder export into a full cache/download manager with background/resumable jobs, provider checksum manifests where available, and deeper system Downloads integration where legally allowed.
