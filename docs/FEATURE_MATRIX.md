...
| Search | Done | Local title/artist/album/genre/source/folder/saved-lyrics filtering plus submitted-query/playback/metadata suggestion chips and Sources-tab provider search fan-out/ranking with local-library merge. | Pagination, typo tolerance, and richer global ranking. |
...
| Multi-provider search | Scaffolded | Spotube, Grayjay, Echo Music | `ProviderSearchCoordinator` fans out to searchable adapters, ranks mixed results, limits per-provider results, resolves metadata-only tracks when supported, captures provider-specific errors, has unit tests, and Sources tab can search the Local Library, Demo Provider, Radio Browser, and Internet Archive together. Offline mode keeps unified search local-library-only instead of contacting network providers. Needed next: provider pagination, authenticated provider opt-in, and richer provider-specific ranking. |
