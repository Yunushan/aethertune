...
| Provider contract test suite | Scaffolded | Shared music-provider capability/disclosure tests cover Demo, Podcast RSS, Radio Browser, Internet Archive, Jellyfin, and Navidrome/Subsonic; dedicated LRCLIB tests cover the separate lyrics-provider endpoint, headers, disclosure, parsing, deduplication, ranking, and failure contract. Needed next: reusable fixture package, provider-specific offline/playback contract cases, and CI reporting per adapter. |
