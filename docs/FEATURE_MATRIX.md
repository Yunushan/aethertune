...
| Podcast RSS subscriptions | Scaffolded | Sources tab adds/removes persisted RSS feed subscriptions, imports/exports OPML, loads playable episodes, tracks refresh status/staleness, plays/saves episodes, queues cache/download requests after provider policy approval, materializes direct enclosure URLs into private cache storage with usage/eviction controls, resumes saved episode progress, includes backups, declares cache/download policy for legal enclosures, and provider/cache/store behavior has tests. | Background/resumable downloads and configurable size limits. |
...
| Stream cache | Scaffolded | InnerTune, RiMusic, YouTube Music | Provider-approved cache requests can be queued, managed, downloaded into private app storage for direct HTTP(S) media URLs, measured, trimmed/cleared, and replayed as local files. Background/resumable fetches remain. |
...
| Cache size limits | Scaffolded | All offline clients | Options can measure private cache usage, clear cached media, and trim private cache files to 500.0 MB. Configurable limits, automatic pressure eviction, and per-provider policies remain. |
...
| Podcast RSS | Scaffolded | YouTube Music podcasts, NewPipe | RSS parser/provider, persisted feed subscriptions, OPML import/export, refresh status/stale policy, episode listing, playback, saved episode progress/resume, library save, backup/restore, cache/download policy declarations, queued offline requests, private direct-enclosure caching, and private cache eviction are implemented; background/resumable downloads and configurable limits remain. |
