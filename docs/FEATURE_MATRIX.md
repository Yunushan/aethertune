...
| Metadata editing | Scaffolded | Namida | Track menus edit persisted library title, artist, album, and genre; search, browse groups, suggestions, playlists, and backup data update from the edited record. Confirmed local MP3 edits write title, artist, album, and genre into ID3v2.3/v2.4 text frames, retain an ID3v1 compatibility tag, preserve supported non-editable v2 frames such as artwork, preserve audio bytes through a transactional copy, and refuse unsupported tag layouts before modifying the file. Focused tests prove Unicode scanner round-trips, frame preservation, audio preservation, and failure safety. Needed next: FLAC/M4A/WAV writing, artwork editing, scanner reconciliation, and rollback recovery after an external filesystem failure. |
