...
| Stream cache | Scaffolded | InnerTune, RiMusic, YouTube Music | Provider-approved cache requests can be queued, managed, downloaded into private app storage for direct HTTP(S) media URLs, measured, trimmed/cleared to configurable app/provider size limits, automatically evicted after cache writes when over limit, and replayed as local files. Background/resumable fetches remain. |
...
| Podcast RSS | Scaffolded | YouTube Music podcasts, NewPipe | RSS parser/provider, persisted feed subscriptions, OPML import/export, refresh status/stale policy, episode listing, playback, saved episode progress/resume, library save, backup/restore, cache/download policy declarations, queued offline requests, private direct-enclosure caching, and app/provider quota private cache eviction are implemented; background/resumable downloads remain. |
...
7. Expand the current direct-URL offline cache into a full cache/download manager with background/resumable jobs, checksum validation, and optional public export where legally allowed.
