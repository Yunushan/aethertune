...
| Stats / recap | Scaffolded | YouTube Music, Namida | Store-level stats aggregate local tracks, favorites, play counts, estimated listening duration, and top tracks/artists/albums/genres; History tab filters stats by all time, last 7 days, last 30 days, or last year, shows monthly/yearly calendar recap cards, exports each recap as a fixed-size PNG through the native save dialog, exports the selected range as JSON/CSV, supports deleting individual play entries from the filtered history list, searches recently played tracks and individual play entries, and respects the pause-listening-history toggle for new plays. Store/widget tests cover ranking, date filtering, calendar recaps, PNG rendering, exports, pause behavior, entry deletion, and history search. Needed next: richer charts and saved recap themes. |
