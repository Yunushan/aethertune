...
| Metadata editing | Scaffolded | Namida | Track menus edit persisted library title, artist, album, and genre; search, browse groups, suggestions, playlists, and backup data update from the edited record. Local MP3 edits offer explicit confirmation to write title, artist, album, and supported standard genres to an ID3v1 tag; unknown genres remain app-only and the fixed-size writer has focused file tests. Needed next: ID3v2/FLAC/M4A/WAV writing, artwork editing, scanner reconciliation, and rollback handling. |
