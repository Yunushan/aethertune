...
| Podcast RSS subscriptions | Scaffolded | Sources tab adds/removes persisted RSS feed subscriptions, imports/exports OPML, loads playable episodes, tracks refresh status/staleness, plays/saves episodes, queues cache/download requests after provider policy approval, materializes direct enclosure URLs into private cache storage, resumes saved episode progress, includes backups, declares cache/download policy for legal enclosures, and provider/cache/store behavior has tests. | Background/resumable downloads, eviction, and size limits. |
...
| Stream cache | Scaffolded | InnerTune, RiMusic, YouTube Music | Provider-approved cache requests can be queued, managed, downloaded into private app storage for direct HTTP(S) media URLs, and replayed as local files; eviction remains. |
| Download queue | Scaffolded | YouTube Music, NewPipe | Sources tab queues legal downloads only after `OfflineMediaPolicy` allows them, persists the queue, exposes remove/clear/cache management in Options, and materializes direct URLs into private storage. Public Downloads export is not implemented. |
...
| Podcast RSS | Scaffolded | YouTube Music podcasts, NewPipe | RSS parser/provider, persisted feed subscriptions, OPML import/export, refresh status/stale policy, episode listing, playback, saved episode progress/resume, library save, backup/restore, cache/download policy declarations, queued offline requests, and private direct-enclosure caching are implemented; background/resumable downloads and eviction remain. |
