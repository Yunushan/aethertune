...
| Manual playlists | Done | Persisted user playlists support add/remove/find/reorder/import/export/play, folders with All/Unfiled filtering and move-to-folder actions, plus private local-image or web artwork in the list and playlist sheet. Native JSON/M3U/CSV file save/open and view/paste fallbacks are implemented. Store tests cover artwork and folder persistence, portable JSON, and backup restore. | Artwork cropper, generated collage artwork, and richer collaboration. |
...
| Playlist artwork | Scaffolded | YouTube Music, Namida, RiMusic | Manual playlist menu accepts a validated local PNG/JPEG/GIF/WebP image copied into private app storage, or an http/https artwork URL, and can remove either. Playlist cards render local/network artwork with a stable fallback. Private local image paths are stripped from backup, portable playlist, and sync documents; web artwork remains portable. Needed next: cropper, generated collages from tracks, image cache policy, and cross-device artwork sync. |
