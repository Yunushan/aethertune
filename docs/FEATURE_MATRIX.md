...
| Metadata editing | Scaffolded | Namida | Track menus edit persisted library title, artist, album, and genre; search, browse groups, suggestions, playlists, and backup data update from the edited record. Confirmed local MP3 edits write title, artist, album, and genre into ID3v2.3/v2.4 text frames, retain an ID3v1 compatibility tag, preserve supported non-editable v2 frames such as artwork, preserve audio bytes through a transactional copy, and refuse unsupported tag layouts before modifying the file. Confirmed local FLAC edits update standard Vorbis comments, preserve non-comment metadata blocks such as artwork plus custom comments and audio bytes through the same transactional copy pattern, and reject malformed comments before mutation. Confirmed local WAV edits update RIFF INFO fields, preserve unrelated RIFF chunks/custom INFO fields and data bytes transactionally, and reject malformed INFO lists before mutation; legacy INFO text falls back to `?` outside Latin-1. Focused tests prove scanner round-trips, frame/block/chunk preservation, audio preservation, and failure safety. Needed next: M4A writing, artwork editing, scanner reconciliation, and rollback recovery after an external filesystem failure. |
