...
| Stored metadata editing | Scaffolded | Track menus edit saved title, artist, album, and genre, and local tracks can replace embedded artwork with a validated private PNG/JPEG/GIF/WebP file or an http/https URL, then restore scanned artwork. Manual file artwork stays on the current device; portable backup/sync/playlist documents retain safe web or embedded artwork and never expose private file paths. Store/file-store tests cover persistence, restore, and privacy filtering. | M4A artwork/tag writing, artwork crop/editing, scanner reconciliation after external edits, and rollback handling. |
