...
| Chapters | Scaffolded | YouTube/NewPipe-style apps | Saved tracks carry normalized, portable chapter markers. Full Now Playing shows the active marker, expands to a timestamped seek list, and provides a validated timestamp/title editor for the saved current track. Legal Podcast RSS feeds propagate inline Podlove Simple Chapters and same-origin Podcasting 2.0 chapter JSON documents into playable episodes; external chapter domains are never requested. Model, provider, store, and widget tests cover parsing, privacy boundaries, persistence, active selection, and seek behavior. Needed next: visual timeline markers and an explicit user-approved policy for third-party chapter hosts. |
