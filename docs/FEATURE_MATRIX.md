...
| Provider contract test suite | Scaffolded | Shared capability/disclosure tests cover Demo, Podcast RSS, Radio Browser, Internet Archive, Jellyfin, and Navidrome/Subsonic; self-hosted tests additionally cover secure metadata/secret separation, connection testing, token/query redaction, runtime URL stripping, restart reconstruction, removal, exact browse/detail/artwork endpoint filters, Jellyfin header authentication, fixed Subsonic token vectors, catalog/artwork parsing, binary image MIME/size gates, artwork cache reuse/invalidation, in-memory rendering, phone/desktop navigation, play/save actions, retry, offline zero-request behavior, and native wrapper requirements. Dedicated LRCLIB tests cover its separate lyrics contract. Needed next: reusable fixture package, live opt-in integration fixtures, and CI reporting per adapter. |
...
5. Complete legal providers: local folder watch, background Podcast RSS jobs, richer Radio Browser UX, self-hosted system-media artwork plus remote playlist mutation/sync, and Internet Archive collection pages.
