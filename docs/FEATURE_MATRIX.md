...
| Queue | Done | Current list is loaded once as a native playlist, follows automatic engine index transitions, restores across app launches, preserves current position while reordered/trimmed, filters stream entries in offline mode, and can be saved as a playlist. Controller tests cover native transition mapping, next/previous without reloads, offline filtering, mutation rebuilds, persistence, and stop-at-end isolation. | Cross-device queue sync. |
