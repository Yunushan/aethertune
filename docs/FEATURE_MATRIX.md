...
| Backup/restore | Done | Versioned JSON backup/restore with native file save/open dialogs and JSON view/paste fallback, including theme/accent preferences, pause-listening-history preference, offline mode, and queued offline media requests. Store and backup-document tests cover the portable format. | Migration tooling and cloud targets. |
...
| Search | Done | Typo-tolerant local title/artist/album/genre/source/folder/saved-lyrics filtering plus submitted-query/playback/metadata suggestion chips and Sources-tab provider search fan-out/ranking with local-library merge and configured Jellyfin/Navidrome/Subsonic accounts. Internet Archive catalogue results paginate explicitly. | Pagination for other providers and richer global ranking. |
...
| Stats / recap | Scaffolded | YouTube Music, Namida | Store-level stats aggregate local tracks, favorites, play counts, estimated listening duration, and top tracks/artists/albums/genres; Home and History show responsive normalized bar charts for top tracks/artists; History filters stats by all time, last 7 days, last 30 days, or last year, persists named range/query views, shows an accessible scrollable day-by-day listening heatmap plus monthly/yearly calendar recap cards, exports each recap as a fixed-size PNG through the native save dialog, exports the selected range as JSON/CSV, supports deleting individual play entries, searches history, and respects the pause-listening-history toggle. Store/widget tests cover ranking, saved-view persistence/backup, chart normalization/compact layout, date filtering, calendar heatmaps/recaps, PNG rendering, exports, pause behavior, entry deletion, and history search. Needed next: more chart dimensions and saved recap themes. |
| Backup/restore | Done | Namida, local-first apps | Includes local library data, theme/accent preferences, pause-listening-history preference, offline mode, and queued offline media requests; native file save/open dialogs use a portable UTF-8 JSON document while view/paste remains available. Cloud targets and migration checks remain. |
...
| Multi-provider search | Scaffolded | Spotube, Grayjay, Echo Music | `ProviderSearchCoordinator` fans out to searchable adapters, ranks mixed results with typo-tolerant metadata scoring, limits per-provider results, resolves metadata-only tracks when supported, captures provider-specific errors, has unit tests, and Sources can search the Local Library, Demo Provider, Radio Browser, Internet Archive, and configured Jellyfin/Navidrome/Subsonic accounts together. Internet Archive exposes independent paginated catalogue browsing with total-aware continuation and retry; configured self-hosted accounts also expose dedicated Artists, Albums, and Playlists tabs with filtering, drill-down, and credential-safe in-app artwork. Offline mode keeps all provider requests local-library-only. Needed next: pagination for remaining providers, richer provider-specific ranking, and browse surfaces for the remaining providers. |
...
| Smart playlists | Scaffolded | Namida, desktop players | Built-in dynamic playlists plus persisted user-created smart rules are implemented for metadata/lyrics search text, exact source/artist/album/genre, favorites-only, minimum/maximum duration, minimum play count, minimum days since last play, sort mode, and result limit. The editor supports nested Match all/Match any rule groups, editable private/web artwork, and artwork fallbacks from matching tracks; private artwork is stripped from portable documents. Bounded AetherTune import links safely copy and import nested dynamic queries without private artwork. Needed next: sync, OS deep-link routing, and server/public sharing. |
| Playlist artwork | Scaffolded | YouTube Music, Namida, RiMusic | Manual playlist menu accepts a validated local PNG/JPEG/GIF/WebP image copied into private app storage, or an http/https artwork URL, and can remove either. Explicit artwork overrides a generated collage from up to four ordered playlist tracks; empty playlists retain a stable fallback. Private local image paths are stripped from backup, portable playlist, and sync documents; web artwork remains portable. Needed next: cropper, image cache policy, and cross-device artwork sync. |
...
| Offline lyrics cache | Scaffolded | InnerTune, Namida | LRCLIB search responses are cached locally by normalized search query. Offline mode invokes a cache-only provider path that cannot contact LRCLIB and reports when no matching cached result exists. Cache expiry/invalidation controls remain. |
...
| Manual lyrics import/edit | Scaffolded | Local library players | Manual editor imports UTF-8 `.txt` and `.lrc` files through the platform file picker, folder scans automatically associate matching basename `.lrc` or `.txt` sidecars without overwriting existing saved lyrics, previews parsed synced LRC lines, saves/deletes per-track lyrics, can select attributed plain/synced LRCLIB results, saves full TXT/LRC export files with suggested filenames, copies export text, and copies bounded attributed share excerpts. Needed next: embedded tag import and batch matching. |
...
| Creator/channel following | Scaffolded | Grayjay, YouTube Music | Users can follow/unfollow normalized local library artist names from an artist browse sheet. Follows persist in device storage, backups, and library sync snapshots; Home shows newest local additions from followed artists. This is not a remote provider/channel subscription or a multi-source creator identity model. |
...
| Share track/album/playlist | Scaffolded | All music apps | Track menus, album/artist/genre/source/folder browse sheets, and playlist menus copy local AetherTune share text for tracks, groups, playlists, and lyrics; local file paths are redacted from share payloads. Each successful copy confirmation now includes a native platform Share action using the same text, with an anchored iPad/macOS presentation origin and unavailable/error states. Manual playlists can copy a bounded `aethertune://playlist` import link that embeds a sync-safe playlist document; the Import sheet accepts pasted links and matches their tracks against the local library without carrying device paths, stream URLs, or private artwork. Now Playing previews and saves a privacy-safe artwork track card as PNG; album browse headers and playlist menus preview and save PNG cards using representative album artwork, explicit playlist artwork, or generated playlist collages; monthly/yearly listening recaps render and save as PNG cards. Needed next: OS-level deep-link routing and server-backed public sharing. |
...
| Library sync | Done | YouTube Music / multi-device needs | The optional Dart server accepts authenticated `GET`/`PUT /api/v1/sync/library` snapshots with size checks, checksums, durable latest-revision storage, and optimistic conflict responses. The client requires HTTPS unless the user explicitly permits HTTP, stores its token securely, exposes manual upload/download plus conflict resolution, and can opt into foreground uploads at most every 15 minutes. On an explicit conflict, **Merge both** fetches the current server revision, unions independent library records, combines matching playlist memberships in local order, selects newer progress/lyrics timestamps, keeps local scalar preferences and same-ID metadata, and uploads the merged result against the fetched revision. Automatic uploads never download, overwrite, or merge; registration, token lifecycle, and background scheduling remain roadmap work. |
