...
| Multi-provider search | Scaffolded | Spotube, Grayjay, Echo Music | `ProviderSearchCoordinator` fans out to searchable adapters, ranks mixed results with typo-tolerant metadata scoring, limits per-provider results, resolves metadata-only tracks when supported, captures provider-specific errors, has unit tests, and Sources can search the Local Library, Demo Provider, Radio Browser, Internet Archive, and configured Jellyfin/Navidrome/Subsonic accounts together. Configured self-hosted accounts also expose dedicated Artists, Albums, and Playlists tabs with filtering and drill-down. Offline mode keeps all provider requests local-library-only. Needed next: provider pagination, richer provider-specific ranking, authenticated artwork, and browse surfaces for the remaining providers. |
...
| Jellyfin | Scaffolded | Self-hosted music users | Sources can add/edit/test/remove HTTPS-by-default Jellyfin accounts, store API keys in the platform credential vault, activate authenticated audio search, and open dedicated Artists, Albums, and Playlists tabs. Artist-to-album and album/playlist-to-track drill-down supports filtering, play all, save all, per-track playback/save, and provider-approved cache/download queueing. Tracks stay metadata-only until runtime stream resolution; credentials are redacted and excluded from queue JSON/backups. Endpoint, parser, secure-store, phone/desktop widget, retry, and offline zero-request behavior have tests. Needed next: authenticated artwork loading, remote playlist create/edit/delete/reorder, token rotation, and cross-device sync. |
| Navidrome/Subsonic | Scaffolded | Self-hosted music users | Sources can add/edit/test/remove HTTPS-by-default Navidrome/Subsonic accounts, store passwords in the platform credential vault, activate `ping.view` testing plus `search3.view` search, and browse documented `getArtists`, `getArtist`, `getAlbumList2`, `getAlbum`, `getPlaylists`, and `getPlaylist` responses through dedicated Artists, Albums, and Playlists tabs. Drill-down supports filtering, play/save, and provider-approved offline queueing while `stream.view` credentials remain runtime-only. Endpoint, parser, secure-store, phone/desktop widget, retry, and offline zero-request behavior have tests. Needed next: authenticated artwork loading, remote playlist create/edit/delete/reorder, token auth where supported, and cross-device sync. |
...
| Provider credential vault | Scaffolded | Self-hosted/official APIs | Jellyfin API keys and Navidrome/Subsonic passwords use `flutter_secure_storage` across Android, iOS, Linux, macOS, and Windows; account metadata is separate, secrets are excluded from queue/library JSON and backups, deletion removes vault entries, Android backup is disabled, Apple keychain entitlements are generated, and Linux CI/release installs libsecret. Needed next: token refresh/rotation, biometric policy choices, migration/versioning, and physical-device keychain/keystore/credential-manager tests. |
...
| Secure token storage | Scaffolded | Implemented for static Jellyfin and Navidrome/Subsonic credentials through platform-secure storage with HTTPS default, explicit insecure-HTTP consent, redacted errors, and no authenticated URL persistence. OAuth refresh tokens and physical-device storage tests remain. |
...
| Provider contract test suite | Scaffolded | Shared capability/disclosure tests cover Demo, Podcast RSS, Radio Browser, Internet Archive, Jellyfin, and Navidrome/Subsonic; self-hosted tests additionally cover secure metadata/secret separation, connection testing, error redaction, runtime URL stripping, restart reconstruction, removal, exact browse/detail endpoint filters, catalog parsing, phone/desktop navigation, play/save actions, retry, offline zero-request behavior, and native wrapper requirements. Dedicated LRCLIB tests cover its separate lyrics contract. Needed next: reusable fixture package, live opt-in integration fixtures, and CI reporting per adapter. |
...
5. Complete legal providers: local folder watch, background Podcast RSS jobs, richer Radio Browser UX, authenticated Jellyfin/Navidrome artwork and remote playlist mutation/sync, and Internet Archive collection pages.
