...
| Home feed | Scaffolded | YouTube Music, InnerTune, RiMusic | Local Home tab builds continue-listening, recently played, radio seed, most played, favorites, and recently added sections from the on-device library. Needed next: provider-backed feed sections, pagination, refresh controls, and account/provider personalization where legal. |
