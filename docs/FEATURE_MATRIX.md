...
| Stored metadata editing | Scaffolded | Track menus edit saved title, artist, album, and genre, and local tracks can replace artwork with a validated private PNG/JPEG/GIF/WebP file or an http/https URL, then restore scanned artwork. Local M4A tracks can also transactionally replace their embedded cover with a PNG/JPEG under 512 KiB; the UI immediately refreshes the scanned cover while preserving any existing private/web override. Manual file artwork stays on the current device; portable backup/sync/playlist documents retain safe web or embedded artwork and never expose private file paths. Store/file-store and M4A writer tests cover persistence, scanner refresh, preservation, and failure safety. | Artwork crop/editing, scanner reconciliation after unrelated external edits, robust front-loaded-M4A offset repair, and rollback handling. |
