...
| Internet Archive | Scaffolded | ArchiveTune | Public audio metadata search, collection/subject/creator/year filters, facet suggestion chips, dedicated item detail pages with collection/subject/license metadata and all playable files, Sources-tab play/save/cache/download queue, checksum-verified private direct-file caching with HTTP Range retry resume, app/provider quota private cache eviction, cache/download policy declarations, and provider/cache/detail tests are implemented; background download jobs remain. |
