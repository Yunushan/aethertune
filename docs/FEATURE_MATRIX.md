...
| Feature | Status | Inspired by | Needed to add |
|---|---:|---|---|
| Local file import | Done | Namida, Musify | File picker import and recursive folder scanning are implemented, including common filename metadata parsing, basic ID3v1 MP3 tag parsing, and basic ID3v2 MP3 text-frame parsing; folder watch, FLAC/M4A tags, artwork extraction, and scoped-storage polish remain. |
...
| Metadata scanner | Scaffolded | Namida | Folder import derives album/folder metadata from parent folders, parses common filename patterns such as leading track numbers and `Artist - Title`, reads basic ID3v1 MP3 title/artist/album tags, and reads basic ID3v2 MP3 title/artist/album/genre text frames before falling back to filenames; scanner tests cover recursive import, dashed titles, ID3v1/ID3v2 tag preference, UTF-16 ID3v2 text, and tag fallback. Needed next: FLAC/M4A tags, artwork extraction, background indexing. |
...
| Album/artist/genre/source/folder views | Done | YouTube Music, Namida | Library browse sheets group tracks by artist, album, genre, source, and imported folder; metadata scanner now reads basic ID3v2 genre text frames, while FLAC/M4A genre tags and richer tag formats remain. |
...
|---|---:|---|---|
| Local files | Done | Namida, Musify | Recursive folder scanner, filename metadata parser, basic ID3v1 MP3 title/artist/album tag parser, and basic ID3v2 MP3 title/artist/album/genre text-frame parser are implemented; FLAC/M4A tags, artwork extraction, richer tag parsing, and folder watch remain. |
