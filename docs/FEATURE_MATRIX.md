...
|---|---:|---|---|
| Local files | Done | Namida, Musify | Recursive folder scanner, filename metadata parser, matching `.lrc`/`.txt` lyric sidecar association, basic ID3v1 MP3 title/artist/album tag parser, basic ID3v2 MP3 title/artist/album/genre text-frame and APIC/PIC artwork parser, basic FLAC Vorbis comment title/artist/album/genre and picture artwork parser, basic M4A title/artist/album/genre atom plus `covr` artwork parser, and basic WAV RIFF INFO title/artist/album/genre parser are implemented; richer tag parsing, artwork editing, and folder watch remain. |
| Demo provider | Done | Provider template | Developer docs and test fixture provider. |
| Jellyfin | Scaffolded | Self-hosted music users | Sources can add/edit/test/remove HTTPS-by-default Jellyfin accounts, store API keys in the platform credential vault, activate authenticated audio search, and open dedicated Artists, Albums, and Playlists tabs. Artist-to-album and album/playlist-to-track drill-down supports filtering, play all, save all, per-track playback/save, and provider-approved cache/download queueing. Safe image IDs/tags persist with metadata; `/Items/{id}/Images/Primary` bytes use `X-Emby-Token` internally, enforce image MIME plus a 10 MiB limit, and render through a 128-entry memory cache in browse rows, saved library tiles, the mini-player, and Now Playing without exposing credential URLs to Flutter's image cache. Playback materializes validated bytes through atomic, format-aware hashed files under a bounded private cache for Android notification and Apple lock-screen/Control Center artwork; runtime file URIs are omitted from JSON and account removal deletes that provider's files. Endpoint, parser, binary-loader, secure-store cache/invalidation, file-cache, serialization, system-media, phone/desktop widget, retry, and offline zero-request behavior have tests. Needed next: remote playlist create/edit/delete/reorder, API-key rotation, and cross-device sync. |
| Navidrome/Subsonic | Scaffolded | Self-hosted music users | Sources can add/edit/test/remove HTTPS-by-default Navidrome/Subsonic accounts, store passwords in the platform credential vault, activate `ping.view` testing plus `search3.view` search, and browse documented `getArtists`, `getArtist`, `getAlbumList2`, `getAlbum`, `getPlaylists`, and `getPlaylist` responses. Every request now uses the documented random-salt `t=md5(password+salt)` flow instead of reversible `p=enc:` authentication. Safe `coverArt` IDs call `getCoverArt.view` only inside the adapter; image MIME/10 MiB limits and a 128-entry memory cache keep credential URLs out of persistence and Flutter's image cache while rendering browse, library, mini-player, and Now Playing artwork. Playback materializes validated bytes through atomic, format-aware hashed files under a bounded private cache for system media artwork; runtime file URIs are omitted from JSON and account removal deletes that provider's files. Needed next: remote playlist create/edit/delete/reorder, credential rotation UX, and cross-device sync. |
...
| Secure token storage | Scaffolded | Implemented for static Jellyfin and Navidrome/Subsonic credentials through platform-secure storage with HTTPS default, explicit insecure-HTTP consent, redacted errors, no authenticated URL persistence, and random-salt Subsonic request tokens that avoid transmitting the reversible encoded password. OAuth refresh tokens, credential rotation UX, and physical-device storage tests remain. |
...
| Provider contract test suite | Scaffolded | Shared capability/disclosure tests cover Demo, Podcast RSS, Radio Browser, Internet Archive, Jellyfin, and Navidrome/Subsonic; self-hosted tests additionally cover secure metadata/secret separation, connection testing, token/query redaction, ephemeral stream/artwork URL stripping, restart reconstruction, removal, exact browse/detail/artwork endpoint filters, Jellyfin header authentication, fixed Subsonic token vectors, catalog/artwork parsing, binary image MIME/size gates, memory-cache reuse/invalidation, private artwork file format/path/reuse/pruning/stale-part/account cleanup, system-media publication, in-memory rendering, phone/desktop navigation, play/save actions, retry, offline zero-request behavior, and native wrapper requirements. Dedicated LRCLIB tests cover its separate lyrics contract. Needed next: reusable fixture package, live opt-in integration fixtures, and CI reporting per adapter. |
...
5. Complete legal providers: local folder watch, background Podcast RSS jobs, richer Radio Browser UX, self-hosted remote playlist mutation/sync, and Internet Archive collection pages.
...
11. Extend the implemented client provider vault with token refresh/rotation, then build server auth, sync, playlist/library/playback-position APIs, and self-hosting docs.
