...
| Stream cache | Scaffolded | InnerTune, RiMusic, YouTube Music | Provider-approved cache requests can be queued, managed, downloaded into private app storage for direct HTTP(S) media URLs, checksum-verified after write, stored with byte-count/checksum metadata, measured, trimmed/cleared to configurable app/provider size limits, automatically evicted after cache writes when over limit, and replayed as local files. Background/resumable fetches remain. |
| Download queue | Scaffolded | YouTube Music, NewPipe | Sources tab queues legal downloads only after `OfflineMediaPolicy` allows them, persists the queue with cache metadata, exposes remove/clear/cache management in Options, materializes direct URLs into checksum-verified private storage, and can evict private cached files. Public Downloads export is not implemented. |
...
7. Expand the current checksum-verified direct-URL offline cache into a full cache/download manager with background/resumable jobs, provider checksum manifests where available, and optional public export where legally allowed.
