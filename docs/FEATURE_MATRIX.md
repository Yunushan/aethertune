...
| Podcast RSS subscriptions | Scaffolded | Sources tab adds/removes persisted RSS feed subscriptions, imports/exports OPML, loads playable episodes, tracks refresh status/staleness, plays/saves episodes, queues cache/download requests after provider policy approval, materializes direct enclosure URLs into private cache storage with usage/configurable-limit eviction controls, resumes saved episode progress, includes backups, declares cache/download policy for legal enclosures, and provider/cache/store behavior has tests. | Background/resumable downloads and per-provider cache policies. |
...
| Stream cache | Scaffolded | InnerTune, RiMusic, YouTube Music | Provider-approved cache requests can be queued, managed, downloaded into private app storage for direct HTTP(S) media URLs, measured, trimmed/cleared to a configurable app-level size limit, automatically evicted after cache writes when over limit, and replayed as local files. Background/resumable fetches remain. |
| Download queue | Scaffolded | YouTube Music, NewPipe | Sources tab queues legal downloads only after `OfflineMediaPolicy` allows them, persists the queue, exposes remove/clear/cache management in Options, materializes direct URLs into private storage, and can evict private cached files. Public Downloads export is not implemented. |
| Cache size limits | Done | All offline clients | Options can measure private cache usage, set a persisted 50-51200 MB private cache limit, trim private cache files to that limit, clear cached media, automatically evict the oldest private cached files after successful cache writes when usage exceeds the limit, and preserve the limit through backup/restore. | Per-provider quota policies remain. |
...
| Podcast RSS | Scaffolded | YouTube Music podcasts, NewPipe | RSS parser/provider, persisted feed subscriptions, OPML import/export, refresh status/stale policy, episode listing, playback, saved episode progress/resume, library save, backup/restore, cache/download policy declarations, queued offline requests, private direct-enclosure caching, and configurable-limit private cache eviction are implemented; background/resumable downloads and per-provider cache policies remain. |
...
| Internet Archive | Scaffolded | ArchiveTune | Public audio metadata search, collection/subject/creator/year filters, playable file resolver, multi-file item results, Sources-tab play/save/cache/download queue, private direct-file caching, configurable-limit private cache eviction, cache/download policy declarations, and provider/cache tests are implemented; collection browse pages, facet suggestion UI, background/resumable downloads, and per-provider cache policies remain. |
...
7. Expand the current direct-URL offline cache into a full cache/download manager with background/resumable jobs, per-provider quotas, checksum validation, and optional public export where legally allowed.
