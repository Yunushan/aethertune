...
| Smart playlists | Scaffolded | Playlists tab exposes built-in dynamic Favorites, Recently added, Recently played, and Most played collections plus persisted custom smart playlists with a Match all/Match any control over search text, exact source ID, artist, album, genre, optional minimum/maximum duration in seconds, favorites-only, minimum play count, and minimum days since last play (including never-played tracks), plus sort and result limits. Store tests cover matching modes, persistence, update/delete, and backup restore. | Nested rule groups, sync, artwork, and cross-device dynamic queries. |
...
| Smart playlists | Scaffolded | Namida, desktop players | Built-in dynamic playlists plus persisted user-created smart rules are implemented for metadata/lyrics search text, exact source/artist/album/genre, favorites-only, minimum/maximum duration, minimum play count, minimum days since last play, sort mode, and result limit; the editor now chooses whether active criteria must all match or may match any. Needed next: nested rule groups, artwork, sync, and shared dynamic queries. |
