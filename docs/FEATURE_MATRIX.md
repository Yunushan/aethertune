...
| Smart playlists | Scaffolded | Namida, desktop players | Built-in dynamic playlists plus persisted user-created smart rules are implemented for metadata/lyrics search text, exact source/artist/album/genre, favorites-only, minimum/maximum duration, minimum play count, minimum days since last play, sort mode, and result limit. The editor supports nested Match all/Match any rule groups, editable private/web artwork, and artwork fallbacks from matching tracks; private artwork is stripped from portable documents. Needed next: sync and shared dynamic queries. |
