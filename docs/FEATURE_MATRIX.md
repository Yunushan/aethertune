...
| Listening history | Done | YouTube Music, Last.fm-style clients | Playback history, range exports, clear history, per-play entry deletion, and pause-listening-history privacy control are implemented; richer history search remains. |
| Stats / recap | Scaffolded | YouTube Music, Namida | Store-level stats aggregate local tracks, favorites, play counts, estimated listening duration, and top tracks/artists/albums/genres; History tab filters stats by all time, last 7 days, last 30 days, or last year, shows monthly/yearly calendar recap cards, exports the selected range as JSON/CSV, supports deleting individual play entries from the filtered history list, and respects the pause-listening-history toggle for new plays. Store tests cover ranking, date filtering, calendar recaps, exports, pause behavior, and entry deletion. Needed next: shareable recap visuals, richer charts, and history search. |
