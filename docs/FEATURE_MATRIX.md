...
| Jellyfin | Scaffolded | Self-hosted music users | Sources can add/edit/test/remove HTTPS-by-default Jellyfin accounts, store API keys in the platform credential vault, activate authenticated audio search, resolve runtime-only stream URLs, redact credential-bearing failures, and reconstruct playback after restart without writing keys to queue JSON/backups. Provider parsing, disclosure, cache/download declarations, native wrapper configuration, and security boundaries have tests. Needed next: dedicated library/album/artist browse pages, authenticated artwork loading, remote playlists, token rotation, and cross-device sync. |
| Navidrome/Subsonic | Scaffolded | Self-hosted music users | Sources can add/edit/test/remove HTTPS-by-default Navidrome/Subsonic accounts, store passwords in the platform credential vault, activate `ping.view` testing plus `search3.view` search, resolve runtime-only `stream.view` URLs, redact raw/reversibly encoded credentials, and reconstruct playback after restart without writing passwords to queue JSON/backups. Needed next: dedicated browse pages, authenticated artwork loading, remote playlists, token auth where supported, and cross-device sync. |
...
| Provider contract test suite | Scaffolded | Shared capability/disclosure tests cover Demo, Podcast RSS, Radio Browser, Internet Archive, Jellyfin, and Navidrome/Subsonic; self-hosted tests additionally cover secure metadata/secret separation, connection testing, error redaction, runtime URL stripping, restart reconstruction, removal, and native wrapper requirements. Dedicated LRCLIB tests cover its separate lyrics contract. Needed next: reusable fixture package, live opt-in integration fixtures, and CI reporting per adapter. |
...
5. Complete legal providers: local folder watch, background Podcast RSS jobs, richer Radio Browser UX, Jellyfin/Navidrome browse and playlist surfaces, and Internet Archive collection pages.
