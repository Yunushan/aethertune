...
| Chapters | Scaffolded | YouTube/NewPipe-style apps | Saved tracks carry normalized, portable chapter markers. Full Now Playing shows the active marker, expands to a timestamped seek list, and provides a validated timestamp/title editor for the saved current track. Legal Podcast RSS feeds with inline Podlove Simple Chapters propagate timestamped markers into playable episodes. Model, provider, store, and widget tests cover parsing, persistence, active selection, and seek behavior. Needed next: Podcasting 2.0 chapter URL loading and visual timeline markers. |
