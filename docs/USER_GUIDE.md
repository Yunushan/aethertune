...
Open a track menu and choose **Edit metadata** to update the saved title, artist, album, or genre. These edits update library search, browse groups, playlists, suggestions, and backups. For local MP3, FLAC, and WAV files, AetherTune separately asks whether to write the values to the file. MP3 writing uses ID3v2.3/v2.4 text frames so Unicode metadata and custom genres survive, keeps an ID3v1 compatibility tag for older players, preserves supported artwork and other non-editable ID3v2 frames, and aborts before changing files with unsupported tag layouts. FLAC writing updates standard Vorbis comments while preserving non-comment metadata blocks, including artwork, custom comments, and audio bytes; malformed comments are left unchanged. WAV writing updates RIFF INFO fields while preserving unrelated RIFF chunks, custom INFO fields, and audio bytes; malformed INFO lists are left unchanged, and legacy INFO text uses `?` for characters outside Latin-1.
...
- Jellyfin: choose **Add Jellyfin**, enter a display name, complete server URL, Jellyfin user ID, and API key, then choose **Test and save**. A successful account joins provider search. Select its account card, or choose **Browse library** from its menu, to open filterable **Artists**, **Albums**, and **Playlists** tabs. Artists open their albums; albums and playlists open tracks. Authenticated artwork appears in those views, saved library rows, the mini-player, Now Playing, and system media surfaces without placing the API key in an image URL. In **Playlists**, use **Create playlist** or a playlist's menu to rename/delete it. From an album track's menu choose **Add to remote playlist**; inside a playlist, use track menus to move tracks up/down or remove them after confirmation. Use **Play all**, **Save all**, a track row, or its menu to play, save, queue an offline cache, or queue a download. Use **Edit and test** for non-secret account metadata, **Rotate credential** to confirm and test a replacement API key before the vault changes, or **Remove** to delete the account. Cross-device playlist sync remains roadmap work.
- Navidrome/Subsonic: choose **Add Navidrome**, enter a display name, complete server URL, username, and password, then choose **Test and save**. AetherTune verifies the account through `ping.view`, includes it in provider search, and exposes the same filterable **Artists**, **Albums**, and **Playlists** browser from the account card or **Browse library** menu. Requests derive a fresh salted token from the vault password instead of transmitting the reversible encoded password. Artist, album, playlist, play/save, artwork, offline-queue, and remote playlist create/rename/delete/add/remove/reorder actions use the documented Subsonic endpoints; selected songs resolve through `stream.view` only when playback or caching needs a stream, and validated artwork reaches system media surfaces through a private local file. Use **Rotate credential** to confirm and test a new password before replacing the vault value. Cross-device playlist sync remains roadmap work.
