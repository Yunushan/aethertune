...
Open a track menu in the Library tab and choose **Lyrics**. You can save plain text lyrics, import a UTF-8 `.txt` or `.lrc` file, paste LRC timestamped lyrics, or choose **Search online** to search LRCLIB. The search sheet discloses the contacted domain and metadata sent, ranks plain and synced matches against the track metadata and duration, and lets you select one result; instrumental records without lyrics cannot be selected. The selected lyrics are stored locally with LRCLIB provider/record attribution, shown in the editor and now-playing lyrics header, included in backups, and added to copied share excerpts. Typing into or importing over provider lyrics changes the source back to manual. Online search is disabled while Offline mode is active. You can also copy the full lyrics export text with a suggested `.txt` or `.lrc` filename, edit or delete lyrics later, and copy a bounded lyrics share excerpt. Folder import attaches matching sidecars such as `Song.lrc` or `Song.txt` beside `Song.mp3`, preferring LRC when both exist. Saved lyrics are included in Library, playlist, and custom smart playlist searches. While a track is playing, use the lyrics button in the player bar to open now-playing lyrics; LRC lines highlight and scroll with playback.
