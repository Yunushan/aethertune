...
- Jellyfin: choose **Add Jellyfin**, enter a display name, complete server URL, Jellyfin user ID, and API key, then choose **Test and save**. A successful account joins provider search and resolves selected tracks for playback. Use the account menu to edit/retest or remove it. Dedicated server browse pages, authenticated artwork, remote playlists, and sync remain roadmap work.
- Navidrome/Subsonic: choose **Add Navidrome**, enter a display name, complete server URL, username, and password, then choose **Test and save**. AetherTune verifies the account through `ping.view`, includes it in provider search, and resolves selected songs through `stream.view`. Dedicated browse pages, authenticated artwork, remote playlists, and sync remain roadmap work.
- More self-hosted/open providers: roadmap.
- Commercial services: official APIs only.

Self-hosted accounts require HTTPS by default. If a LAN-only server uses plain HTTP, AetherTune shows a specific warning and requires explicit consent because credentials would travel without TLS. Account metadata is stored separately from secrets; API keys/passwords use the operating system credential vault and are not included in AetherTune JSON backups. Authenticated stream URLs exist only while preparing playback, are stripped from persisted queues, and are rebuilt from the vault after restart. Removing an account deletes its vault entry and queued tracks. If the credential vault is unavailable, account actions remain disabled and Sources shows the storage error.
