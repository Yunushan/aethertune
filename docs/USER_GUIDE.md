...
Open the Options tab and choose **Export backup** to view a versioned JSON backup. Choose **Restore backup** and paste a backup JSON to replace the local library, playlists, lyrics, history, and podcast feed subscriptions with that backup.
...
- Podcast RSS: add a legal RSS feed URL in Sources, import/export OPML, then load, play, save, or remove feed episodes.
