...
- Jellyfin: choose **Add Jellyfin**, enter a display name, complete server URL, Jellyfin user ID, and API key, then choose **Test and save**. A successful account joins provider search. Select its account card, or choose **Browse library** from its menu, to open filterable **Artists**, **Albums**, and **Playlists** tabs. Artists open their albums; albums and playlists open tracks. Use **Play all**, **Save all**, a track row, or its menu to play, save, queue an offline cache, or queue a download. Use the account menu to edit/retest or remove it. Authenticated artwork, remote playlist editing, and sync remain roadmap work.
- Navidrome/Subsonic: choose **Add Navidrome**, enter a display name, complete server URL, username, and password, then choose **Test and save**. AetherTune verifies the account through `ping.view`, includes it in provider search, and exposes the same filterable **Artists**, **Albums**, and **Playlists** browser from the account card or **Browse library** menu. Artist, album, playlist, play/save, and offline-queue actions use the documented Subsonic endpoints; selected songs resolve through `stream.view` only when playback or caching needs a stream. Authenticated artwork, remote playlist editing, and sync remain roadmap work.
- More self-hosted/open providers: roadmap.
- Commercial services: official APIs only.

Self-hosted accounts require HTTPS by default. If a LAN-only server uses plain HTTP, AetherTune shows a specific warning and requires explicit consent because credentials would travel without TLS. Account metadata is stored separately from secrets; API keys/passwords use the operating system credential vault and are not included in AetherTune JSON backups. Browse and search results contain metadata only. Authenticated stream URLs exist only while preparing playback or an approved offline request, are stripped from persisted queues, and are rebuilt from the vault after restart. Offline mode disables account browsing without issuing catalog requests. Removing an account deletes its vault entry and queued tracks. If the credential vault is unavailable, account actions remain disabled and Sources shows the storage error.
